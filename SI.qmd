...
#| echo: false
#| warning: false
#| fig-cap: Correlation between all functions (untransformed data).
#| fig-width: 12
#| fig-height: 12
...
```{r}
#| label: fig-correlation-contrast-plot
#| echo: false
#| warning: false
#| fig-cap: Correlation between ecosystem functions (contrast to bare ground, FGB).
#| fig-width: 12
#| fig-height: 12

tar_read(correlation_contrast_plot)
...
big_data |>
  group_by(group) |>
  summarise(response = paste(sort(unique(response)), collapse = ", "), .groups = "drop") |>
  gt() |>
  tab_header(title = "Ecosystem functions by group") |>
  cols_label(group = "Group", response = "Ecosystem functions / responses")
...
tar_read(all_functions_pca_plot)
