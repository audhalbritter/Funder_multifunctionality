...
all_plots_list <- significant_responses |>
  set_names() |>
  map(function(resp) {
    make_response_plot(resp, big_data, model_results, 
                      temp_colour, prec_linetype, prec_shape)
  }) |>
...
```{r}
#| label: fig-all-responses
#| echo: false
#| warning: false
#| fig-width: 10
#| fig-height: 6

# Display all plots in order (sorted by group)
for (resp in names(all_plots_list)) {
  print(all_plots_list[[resp]])
}
